...
impl RenderRes {
    pub fn new(owner: &Node2D, def: &Def) -> Self {
        ...
        // Create mesh.
        let mesh_rid = RenderRes::create_basic_mesh(visual_server);
        ...
        render_data.resize(BULK_ARRAY_SIZE);
    ...
    fn create_basic_mesh(visual_server: &VisualServer) -> Rid {
        ...
        arr.resize(VisualServer::ARRAY_MAX as i32);
        arr.set(VisualServer::ARRAY_VERTEX as i32, vertices);
        arr.set(VisualServer::ARRAY_TEX_UV as i32, uvs);
        arr.set(VisualServer::ARRAY_COLOR as i32, colors);
        arr.set(VisualServer::ARRAY_INDEX as i32, indices);
