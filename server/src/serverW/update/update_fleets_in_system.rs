    ...
    for i in 0..s.fleets.len() {
        ...
        if let Some(system_id) = in_system {
            ...
            if let Some(index) = s.systems.get_index(*system_id) {
                let (system_position, system_radius) = query!(s.systems.container(), index, System::position, System::radius);
                if !Collider::new(*system_position, *system_radius, ()).intersection_test_point(*position) {
