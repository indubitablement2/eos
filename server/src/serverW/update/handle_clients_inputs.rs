...
pub fn handle_clients_inputs(
            ...
            match connection.try_recv() {
                Ok(buffer) => {
                    match ClientPacket::deserialize(&buffer) {
                        ...
                        ClientPacket::MetascapeWishPos { wish_pos, movement_multiplier } => {
                        ...
                        ClientPacket::BattlescapeInput { wish_input, last_acknowledge_command } => {
                            todo!()
                        }
                    }
                }
    ...
    for (client_id, fleet_index) in disconnected.into_iter() {
        s.clients.swap_remove_by_id(client_id).expect("There should be a client");
