...
use metascape::{
    offline::OfflineConnectionsManager, offline_client::OfflineConnectionClientSide, Metascape,
};
...
impl MetascapeManager {
    pub fn new(
        ...
        client_id: ClientId,
    ) -> (Self, OfflineConnectionClientSide) {
        ...
        // Create the channels to communicate to the runner thread.
...
fn runner(
    ...
    loop {
        ...
        if send_debug_info {
            let _ = signal_sender.try_send(MetascapeSignal::DebugInfo(metascape_debug_info_str(
                &metascape,
            )));
        }
