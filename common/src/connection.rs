...
pub trait Packet: Sized {
    ...
    fn parse_text(text: String) -> Result<Self, &'static str>;
}

/// Send packets to the remote peer without blocking/async.
///
...
pub struct ConnectionOutbound {
    ...
    outbound_sender: tokio::sync::mpsc::UnboundedSender<Option<Message>>,
}
impl ConnectionOutbound {
    ...
    fn new(socket: WebSocketStream<TcpStream>) -> (Self, ConnectionInbound) {
        ...
        tokio::spawn(async move {
            while let Some(msg) = outbound_receiver.recv().await {
                if let Some(msg) = msg {
                    if let Err(e) = outbound.feed(msg).await {
                        log::debug!("Error feeding message: {}", e);
                        break;
                    }
                } else if let Err(e) = outbound.flush().await {
                    log::debug!("Error flushing messages: {}", e);
                    break;
                }
            }
        });
    ...
    pub fn queue_binary(&self, buf: Vec<u8>) {
        let _ = self.outbound_sender.send(Some(Message::Binary(buf)));
    }

    pub fn queue_text(&self, text: String) {
        let _ = self.outbound_sender.send(Some(Message::Text(text)));
    }

    pub fn queue(&self, packet: impl Packet) {
        self.queue_binary(packet.serialize());
    }

    pub fn flush(&self) {
        let _ = self.outbound_sender.send(None);
    }

    /// Send and flush the message immediately.
    /// Prefer using `queue` and `flush` if possible.
...
impl Drop for ConnectionOutbound {
    fn drop(&mut self) {
        let _ = self
            .outbound_sender
            .send(Some(Message::Close(Some(CloseFrame {
...
impl ConnectionInbound {
    /// Will return `None` if connection is closed
    /// (outbound side dropped or peer disconnected).
    pub async fn recv<T: Packet>(&mut self) -> Option<T> {
        while let Some(msg) = self.inbound.next().await {
            match msg {
                Ok(msg) => match msg {
                    Message::Text(text) => match T::parse_text(text) {
                        Ok(t) => return Some(t),
                        Err(err) => {
                            log::debug!("Failed to parse text message: {}", err);
                            return None;
                        }
                    },
                    Message::Binary(buf) => match T::parse_buf(buf) {
                        Ok(t) => return Some(t),
                        Err(err) => {
                            log::debug!("Failed to parse binary message: {}", err);
                            return None;
                        }
                    },
